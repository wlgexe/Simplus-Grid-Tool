...
for i = 1:length(ApparatusType)
    if ((ApparatusType{i} >= 0) && (ApparatusType{i} <= 9)) || (ApparatusType{i} == 90) || ((ApparatusType{i} >= 20) && (ApparatusType{i} <= 40)) || (ApparatusType{i} == 50) ||((ApparatusType{i} >= 60) && (ApparatusType{i} <= 69))
        vbus(l) = i;    
        l = l+1;
